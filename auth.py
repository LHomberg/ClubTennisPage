...
def signup(): # define the sign up function
    if request.method=='GET': # If the request is GET we return the sign up page and forms
        ...
        if user: # if a user is found, we want to redirect back to signup page so user can try again
            ...
            return redirect(url_for('auth.signup'))
        if "wisc.edu" not in email:
            flash('User must have a wisc.edu address')
            return redirect(url_for('auth.signup'))
        # create a new user with the form data. Hash the password so the plaintext version isn't saved.
        new_user = User(email=email, name=name, password=generate_password_hash(password, method='sha256'), address=address, phone=phone,grade=grade,age=age,team=team,car=car, officer=officer) #
